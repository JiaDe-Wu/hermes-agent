...
def bedrock_agent():
    ...
    agent = AIAgent(
        api_key="test-key",
        # Unroutable on purpose. Post-fix nothing here leaves the process, but the
        # PRE-fix path really did reach agent.base_url over the network -- that is the
        # defect this file exists for. Port 9 (discard) refuses instantly, so the
        # demonstration that these tests fail without the fix stays deterministic and
        # offline instead of depending on a real endpoint answering 401.
...
class TestBedrockIterationLimitSummary:
    ...
    def test_make_client_hook_is_fatal_not_silent(self):
        ...
        from agent.chat_completion_helpers_iteration_limit import _bedrock_summary_no_client

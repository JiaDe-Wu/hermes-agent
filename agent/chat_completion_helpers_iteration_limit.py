"""The iteration-limit summary — one toolless request when the loop hits ``max_iterations``.

Split out of ``chat_completion_helpers`` under ``AGENTS.md``'s ~2,000-line rule; that
owner is 3,569 lines and this is a self-contained unit inside it. Nothing here was
changed by the move: the eleven symbols are the same objects, and the two helpers that
stay behind (``_provider_preferences_for_agent``,
``_merge_nous_portal_messages_extra_body``) are imported inside the functions that need
them, matching this unit's existing idiom for ``relay_llm`` and
``MAX_ITERATIONS_SUMMARY_REQUEST`` and keeping the dependency one-way.

``handle_max_iterations`` is re-exported from ``chat_completion_helpers`` and must stay
reachable there: ``run_agent.py`` binds it with
``agent.lazy_forward.forward("agent.chat_completion_helpers", "handle_max_iterations")``,
which resolves the attribute on that module at EVERY call so tests can patch it, and six
test modules do exactly that.
"""
...
def _anthropic_summary_attempt(agent, api_messages: list, api_request_id: str):
    def _attempt(retry_count: int) -> str:
        ...
        from agent.chat_completion_helpers import _merge_nous_portal_messages_extra_body
